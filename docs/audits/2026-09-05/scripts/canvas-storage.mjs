...
// Persistence conflict across two tabs using the same isolated storage context.
await page.evaluate(async () => {
  const d = { ...p.createStarterPlan(), id: 'two-tabs' };
  s.getState().openRoom(d);
  await repo.saveLocalPlan(d);
});
const second = await ctx.newPage();
await init(second);
await page.evaluate(async () => {
  s.getState().renameRoom('Edited in tab A');
  await repo.saveLocalPlan(s.getState().document);
});
await second.evaluate(async () => {
  s.getState().selectObject('sofa-1');
  s.getState().nudgeSelection({ x: 50, y: 0 });
  await repo.saveLocalPlan(s.getState().document);
});
const conflict = await page.evaluate(async () => ({
  tabAName: s.getState().document.room.name,
  savedName: (await repo.loadLocalPlan()).room.name,
}));
await fs.writeFile(
  (process.env.AUDIT_OUTPUT || './audit-results') + '/multi-tab.json',
  JSON.stringify(conflict, null, 2),
);
console.log({ canvasChecks: canvas.length, conflict });
await b.close();
